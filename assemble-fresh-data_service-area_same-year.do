...
********************************************************************************
forvalues i=12/16 {
	import delimited raw-output\use-same-year-map_service-area\s`i'.csv, clear
	drop if name==" "
	keep x y name id
	gen year=20`i'
...
save newid-fg12-16.dta, replace
}
.
...
merge 1:1 newid year using newid-fg12-16.dta
drop _merge
drop if missing(newid)|missing(year)|newid=="."
compress
save fresh-data_2012-2016_service-area_same-map.dta, replace
erase newid-demo12-16.dta
erase newid_distance_12-16.dta
erase newid-fg12-16.dta
